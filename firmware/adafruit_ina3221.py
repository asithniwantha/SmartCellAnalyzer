...
try:
    ...
    pass
# Removed adafruit_bus_device dependency for MicroPython compatibility
import struct
...
class INA3221:
        ...
        if probe:
            ...
            try:
                ...
                if address not in self.i2c.scan():
                    raise RuntimeError(f"INA3221 not found at address 0x{address:02x}")
            except Exception:
                pass  # Continue without probe check
    ...
    def die_id(self) -> int:
        ...
        """
        data = self._read_register(DIE_ID_REG, 2)
        if data is None:
            return 0
    ...
    def manufacturer_id(self) -> int:
        ...
        """
        data = self._read_register(MANUFACTURER_ID_REG, 2)
        if data is None:
            return 0
        return int.from_bytes(data, "big")
    ...
    def _read_register(self, reg, length):
        try:
            ...
            # Don't print error for every failed read - too verbose
            # print(f"I2C read error: {ex}")
            return None
